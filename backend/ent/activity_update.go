...
func (au *ActivityUpdate) SetCLUBEACTIVITYNAME(s string) *ActivityUpdate {
	au.mutation.SetCLUBEACTIVITYNAME(s)
	return au
}

// AddClubIDs adds the club edge to Club by ids.
func (au *ActivityUpdate) AddClubIDs(ids ...int) *ActivityUpdate {
	au.mutation.AddClubIDs(ids...)
	return au
}

// AddClub adds the club edges to Club.
func (au *ActivityUpdate) AddClub(c ...*Club) *ActivityUpdate {
	ids := make([]int, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return au.AddClubIDs(ids...)
}
...
func (au *ActivityUpdate) RemoveClubIDs(ids ...int) *ActivityUpdate {
	au.mutation.RemoveClubIDs(ids...)
	return au
}

// RemoveClub removes club edges to Club.
func (au *ActivityUpdate) RemoveClub(c ...*Club) *ActivityUpdate {
	ids := make([]int, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return au.RemoveClubIDs(ids...)
}

// Save executes the query and returns the number of rows/vertices matched by this operation.
func (au *ActivityUpdate) Save(ctx context.Context) (int, error) {
	if v, ok := au.mutation.CLUBEACTIVITYNAME(); ok {
		if err := activity.CLUBEACTIVITYNAMEValidator(v); err != nil {
			return 0, &ValidationError{Name: "CLUBE_ACTIVITY_NAME", err: fmt.Errorf("ent: validator failed for field \"CLUBE_ACTIVITY_NAME\": %w", err)}
		}
	}
...
func (au *ActivityUpdate) sqlSave(ctx context.Context) (n int, err error) {
	...
	if value, ok := au.mutation.CLUBEACTIVITYNAME(); ok {
	...
	if nodes := au.mutation.RemovedClubIDs(); len(nodes) > 0 {
...
// SetCLUBEACTIVITYNAME sets the CLUBE_ACTIVITY_NAME field.
func (auo *ActivityUpdateOne) SetCLUBEACTIVITYNAME(s string) *ActivityUpdateOne {
	auo.mutation.SetCLUBEACTIVITYNAME(s)
	return auo
}

// AddClubIDs adds the club edge to Club by ids.
func (auo *ActivityUpdateOne) AddClubIDs(ids ...int) *ActivityUpdateOne {
	auo.mutation.AddClubIDs(ids...)
	return auo
}

// AddClub adds the club edges to Club.
func (auo *ActivityUpdateOne) AddClub(c ...*Club) *ActivityUpdateOne {
	ids := make([]int, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return auo.AddClubIDs(ids...)
}
...
// RemoveClubIDs removes the club edge to Club by ids.
func (auo *ActivityUpdateOne) RemoveClubIDs(ids ...int) *ActivityUpdateOne {
	auo.mutation.RemoveClubIDs(ids...)
	return auo
}

// RemoveClub removes club edges to Club.
func (auo *ActivityUpdateOne) RemoveClub(c ...*Club) *ActivityUpdateOne {
	ids := make([]int, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return auo.RemoveClubIDs(ids...)
}

// Save executes the query and returns the updated entity.
func (auo *ActivityUpdateOne) Save(ctx context.Context) (*Activity, error) {
	if v, ok := auo.mutation.CLUBEACTIVITYNAME(); ok {
		if err := activity.CLUBEACTIVITYNAMEValidator(v); err != nil {
...
func (auo *ActivityUpdateOne) sqlSave(ctx context.Context) (a *Activity, err error) {
	...
	if value, ok := auo.mutation.CLUBEACTIVITYNAME(); ok {
		_spec.Fields.Set = append(_spec.Fields.Set, &sqlgraph.FieldSpec{
			Type:   field.TypeString,
			Value:  value,
			Column: activity.FieldCLUBEACTIVITYNAME,
	...
	if nodes := auo.mutation.RemovedClubIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: false,
			Table:   activity.ClubTable,
