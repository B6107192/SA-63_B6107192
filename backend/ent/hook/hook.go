...
// The ClubTypesFunc type is an adapter to allow the use of ordinary
// function as ClubTypes mutator.
type ClubTypesFunc func(context.Context, *ent.ClubTypesMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ClubTypesFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.ClubTypesMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ClubTypesMutation", m)
	}
	return f(ctx, mv)
}
