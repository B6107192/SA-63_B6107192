...
func (lq *LocationQuery) QueryClub() *ClubQuery {
	query := &ClubQuery{config: lq.config}
...
func (lq *LocationQuery) WithClub(opts ...func(*ClubQuery)) *LocationQuery {
	query := &ClubQuery{config: lq.config}
...
//		GroupBy(location.FieldCLUBELOCATIONNAME).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.Location.Query().
//		Select(location.FieldCLUBELOCATIONNAME).
//		Scan(ctx, &v)
//
...
func (lq *LocationQuery) sqlAll(ctx context.Context) ([]*Location, error) {
	...
	if query := lq.withClub; query != nil {
		...
		query.Where(predicate.Club(func(s *sql.Selector) {
			s.Where(sql.InValues(location.ClubColumn, fks...))
		}))
