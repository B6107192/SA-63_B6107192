...
func (lc *LocationCreate) SetCLUBELOCATIONNAME(s string) *LocationCreate {
	lc.mutation.SetCLUBELOCATIONNAME(s)
	return lc
}

// SetCLUBELOCATIONADDRESS sets the CLUBE_LOCATION_ADDRESS field.
func (lc *LocationCreate) SetCLUBELOCATIONADDRESS(s string) *LocationCreate {
	lc.mutation.SetCLUBELOCATIONADDRESS(s)
	return lc
}

// AddClubIDs adds the club edge to Club by ids.
func (lc *LocationCreate) AddClubIDs(ids ...int) *LocationCreate {
	lc.mutation.AddClubIDs(ids...)
	return lc
}

// AddClub adds the club edges to Club.
func (lc *LocationCreate) AddClub(c ...*Club) *LocationCreate {
	ids := make([]int, len(c))
	for i := range c {
		ids[i] = c[i].ID
	}
	return lc.AddClubIDs(ids...)
}
...
func (lc *LocationCreate) Save(ctx context.Context) (*Location, error) {
	if _, ok := lc.mutation.CLUBELOCATIONNAME(); !ok {
		return nil, &ValidationError{Name: "CLUBE_LOCATION_NAME", err: errors.New("ent: missing required field \"CLUBE_LOCATION_NAME\"")}
	}
	if v, ok := lc.mutation.CLUBELOCATIONNAME(); ok {
	...
	if _, ok := lc.mutation.CLUBELOCATIONADDRESS(); !ok {
		return nil, &ValidationError{Name: "CLUBE_LOCATION_ADDRESS", err: errors.New("ent: missing required field \"CLUBE_LOCATION_ADDRESS\"")}
	}
	if v, ok := lc.mutation.CLUBELOCATIONADDRESS(); ok {
...
func (lc *LocationCreate) createSpec() (*Location, *sqlgraph.CreateSpec) {
	...
	if value, ok := lc.mutation.CLUBELOCATIONNAME(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeString,
			Value:  value,
			Column: location.FieldCLUBELOCATIONNAME,
		})
		l.CLUBELOCATIONNAME = value
	}
	if value, ok := lc.mutation.CLUBELOCATIONADDRESS(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeString,
			Value:  value,
			Column: location.FieldCLUBELOCATIONADDRESS,
		})
		l.CLUBELOCATIONADDRESS = value
	}
	if nodes := lc.mutation.ClubIDs(); len(nodes) > 0 {
