...
func (aq *ActivityQuery) QueryClub() *ClubQuery {
...
func (aq *ActivityQuery) WithClub(opts ...func(*ClubQuery)) *ActivityQuery {
	query := &ClubQuery{config: aq.config}
...
//		GroupBy(activity.FieldCLUBEACTIVITYNAME).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
//
//	client.Activity.Query().
//		Select(activity.FieldCLUBEACTIVITYNAME).
//		Scan(ctx, &v)
//
...
func (aq *ActivityQuery) sqlAll(ctx context.Context) ([]*Activity, error) {
	...
	if query := aq.withClub; query != nil {
		...
		query.Where(predicate.Club(func(s *sql.Selector) {
			s.Where(sql.InValues(activity.ClubColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.activity_id
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "activity_id" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
